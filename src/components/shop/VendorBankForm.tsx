...
import PreviewApplicationDetails from "./PreviewApplicationDetails";
type Props = {
  goBack?: () => void;
};
const VendorBankForm: React.FC<Props> = ({ goBack }) => {
  ...
  const initialValues = {
    bank_name: vendorApplicationPayload.bank_name || user?.bank_name || "",
    account_number:
      vendorApplicationPayload.bank_account_number ||
      user?.bank_account_number ||
      "",
    bank_account_name:
      vendorApplicationPayload.bank_account_name ||
      user?.bank_account_name ||
      "",
    bvn: vendorApplicationPayload.bvn || user?.bvn || "",
  };
  ...
  const save = (values: typeof initialValues) => {
    updateVendorApplicationPayload({
      ...
      bank_account_number: String(values.account_number),
      bvn: String(values.bvn),
    });
    modal.openModal(<PreviewApplicationDetails />);
  };
  return (
