...
const BrandsListTable: React.FC<Props> = ({ brands, isError, isLoading }) => {
  const navigate = useNavigate();
  const { openModal } = useModalStore();
  const handleDelete = (event: Brand) => {
    console.log(event);
  ...
  const handleNewProduct = () => {
    openModal(<NewBrand />, "Create New Brand", "dark");
  };
  ...
  const handleSelectAll = (isChecked: boolean) => {
    setSelectedCategories(isChecked ? brands.map((c) => Number(c.id)) : []);
  };
          ...
          >
            <label className="flex justify-center cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  handleSelectCategory(brand.id, e.target.checked)
                }
            ...
            "
              onClick={() => navigate(`/staff/product/${brand.id}`)}
            >
      ...
      <div className="flex items-center gap-2 px-2 border-b-1 border-b-gray-700 pb-2">
        <label className="flex gap-1 items-center cursor-pointer">
          <input
            ...
            checked={
              selectedCategories.length > 0 &&
              selectedCategories.length === brands.length
            }
